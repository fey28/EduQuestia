...
async function generateContentFromPrompt(prompt, projectId = 'PROJECT_ID') {
...
const checkProfesorRank = (req, res, next) => {
  if (req.user && req.user.rank === 'Profesor') {
    next();
  } else {
    res.sendStatus(403);
  }
...
app.post('/generate', authenticateJWT, checkProfesorRank, async (req, res) => {
...
app.post('/quiz-creator', authenticateJWT, checkProfesorRank, (req, res) => {
...
app.post('/lesson-creator', authenticateJWT, checkProfesorRank, upload.single('file'), (req, res) => {
  ...
  fs.rename(file.path, destPath, (err) => {
    ...
    fs.readFile('users.json', 'utf8', (err, data) => {
      ...
      const users = JSON.parse(data);
      if (!users[username].lessons) {
        users[username].lessons = [];
      }
      users[username].lessons.push(title);
...
app.post('/login', (req, res) => {
  ...
  fs.readFile('users.json', 'utf8', (err, data) => {
    ...
    if (users[username] && users[username].password === password) {
      const token = jwt.sign({ username, rank: users[username].rank }, JWT_SECRET);
      res.send({ success: true, token, rank: users[username].rank });
    } else {
...
app.post('/signup', (req, res) => {
  ...
  fs.readFile('users.json', 'utf8', (err, data) => {
    ...
    users[username] = { password, rank: 'Elev', quizzes: [], lessons: [] };
